...
# The chart tally, where Hugo can read it (see main()). Generated, git-ignored.
BIBLIOMETRICS = ROOT / "data" / "bibliometrics.json"
...
def output_stats(entries: list[dict]) -> dict:
    ...
    dict
        ...
        with the empty slices dropped, plus the two totals. Both renderers — the
        CV and the website — display this one tally, so their numbers cannot
        disagree.

    Raises
    ------
    SystemExit
        If any output type is claimed by no slice or by more than one. That is
        the only way the chart total could stop matching the number of outputs
        there actually are, so it fails the build rather than quietly drifting.
    """
    since = datetime.date.today().year - RECENT_YEARS + 1
    slices = json.loads(OUTPUT_TYPES.read_text(encoding="utf-8"))["slices"]

    claimed = collections.Counter(t for s in slices for t in s["types"])
    ...
    if unclaimed or twice:
        ...
        if unclaimed:
            problems.append(
                f"types no slice claims: {', '.join(unclaimed)}\n"
                "    Add each to a slice's `types` in data/outputtypes.json — until "
                "then\n    the charts would count fewer outputs than the site lists."
            )
        if twice:
            problems.append(f"types claimed by more than one slice: {', '.join(twice)}")
        sys.exit("! data/outputtypes.json — " + "\n! ".join(problems))
    ...
    def tally(chosen: list[dict]) -> list[dict]:
        ...
        for s in slices:
            ...
            if n:
                out.append({
                    "label": s["label"],
                    # The CV is printed and takes the light step; the website
                    # picks per mode, so it needs both.
                    "color": s["light"],
                    "light": s["light"],
                    "dark": s["dark"],
                    "count": n,
    ...
    recent = [e for e in entries if (e["year"] or 0) >= since]
    stats = {"since": since, "all": tally(entries), "recent": tally(recent)}
    # Every type is claimed, so the wedges must add up to every output there is.
    assert sum(s["count"] for s in stats["all"]) == len(entries)
    assert sum(s["count"] for s in stats["recent"]) == len(recent)
    return stats
...
def main() -> int:
    ...
    # The same tally, where Hugo can reach it. The website renders these numbers
    # rather than counting for itself, so the page and the PDF cannot disagree:
    # there is one count, computed here, displayed twice.
    stats = data["output-stats"]
    BIBLIOMETRICS.write_text(
        json.dumps(
            {
                "_comment": "GENERATED by tools/build-cv.py — do not edit, do not commit.",
                "generated": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                "since": stats["since"],
                "total": sum(s["count"] for s in stats["all"]),
                "totalRecent": sum(s["count"] for s in stats["recent"]),
                "all": stats["all"],
                "recent": stats["recent"],
            },
            indent=2,
